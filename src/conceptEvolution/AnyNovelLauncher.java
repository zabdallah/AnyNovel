	...
	private static BLM AnyNovel(BLM BaseModel, Instances StreamInst, HashMap<String, String> parameters)
		...
		for (int i = 0; i < n + 1; i++) {
			...
			for (int p = 0; p < AssignmentArr.length; p++) {
				...
				// // Get the results and build the confusion matrix
				// if (NP.contains("Case 0.1") || NP.contains("Case 0.0") ||
				// NP.contains("Case 0.5")
				// || NP.contains("Case 1.1") || NP.contains("Case 2.1") ||
				// NP.contains("Case 3.2")
				// || NP.contains("Case 2.2") || NP.contains("Case 3.1") ||
				// NP.contains("Case 4.3")
				// || NP.contains("Case 2.3") || NP.contains("Case 3.4") ||
				// NP.contains("Case 4.1")) {
				// // System.out.print("CHECK "+counter_w+" "+NP+"\n");
				// // If last seg is naturally declared by the algorithm,
				// // otherwise, last seg has to be declared anyway (end of
				// // stream).
				// anyDecision = true;
				// // If the last segment is declared by any of these cases
				// if (i == n)
				// declared = true;
				//
				// if (NP.contains("Case 0.0"))
				// s += "**buffer size ";
				// else if (NP.contains("Case 0.1") || NP.contains("Case 0.5"))
				// s += "**Release ";
				// if (NP.contains("Case 0.1") || NP.contains("Case 0.0") ||
				// NP.contains("Case 0.5")) {
				// if (NP.contains("Case 1.1") || NP.contains("Case 2.1") ||
				// NP.contains("Case 3.2")
				// || NP.contains("Case 2.2") || NP.contains("Case 3.1") ||
				// NP.contains("Case 4.3")
				// || NP.contains("Case 2.3") || NP.contains("Case 3.4") ||
				...
				// }
				// // end of stream
				// // get the very last segment, remaining instances and not
				// // declared by any of the declaration cases, needs a
				// // decision
				//
				// if (!declared && i == n & p == AssignmentArr.length - 1) {
				// anyDecision = true;
				...
				// NovelStats.add(pred);
				// System.out.println(pred.getClassSize() + "\n");
				// s = "True label= " + pred.getClassTrueLabel()[0] + "
				// Predicted label= "
				// + pred.getPredictedLabel() + " FA novel= " + pred.IsFAnovel()
				// + " Recurrent Novel= "
				// + pred.isRecurrent() + " The Noelty Type: " +
				// pred.getNoveltyType() + " Size = "
				// + pred.getClassSize();
				...
				// }
				// // Purity of the cluster, reveal the true label to
				// // calculate the purity
				// double av = Double.parseDouble(pred.getClassTrueLabel()[1]) /
				// pred.getClassSize();
				// avPurity += av;
				// pCount++;
				...
				// s += " Purity: " + pred.getClassTrueLabel()[1] + " Max
				// Distance= "
				...
				// s +=
				// CEModel.updateModelConceptEvolution_Unknown(CEModel.getDeclaredData(),
				// pred);
				// updateStr += i + ",";
				// }
				//
				...
				// CEModel.getOldLabels())][ID(
				// pred.getPredictedLabel(), CEModel.getOldLabels())] +=
				// pred.getClassSize();
				// }
				// if (pred.getNoveltyType().contains("Fp")) {
				// strFP = Integer.toString(pred.getClassSize());
				// confusionMatrix[ID(pred.getClassTrueLabel()[0],
				// CEModel.getOldLabels())][CEModel.getOldLabels().length + 1]
				// += pred
				// .getClassSize();
				// }
				// if (pred.isUnknown()) {
				// int sID = ID(pred.getClassTrueLabel()[0],
				// CEModel.getOldLabels());
				// if (sID != -1)
				// confusionMatrix[sID][CEModel.getOldLabels().length] +=
				// pred.getClassSize();
				// }
				// if (pred.getNoveltyType().contains("Tp")) {
				// strTP = Integer.toString(pred.getClassSize());
				// confusionMatrix[CEModel.getOldLabels().length][CEModel.getOldLabels().length
				// + 1] += pred.getClassSize();
				// }
				//
				// sdet = segSize + "," + i + "," + p + "," + strTP + "," +
				// strTN + "," + strFP + "," + strFN
				// + "," + strFA + "," + strTR + "," + strUN;
				// detARR.add(sdet);
				//
				// // validation phase if activated
				// if (validFlag) {
				//
				...
				//
				// }
				// outBuff.append(updateStr);
				// outBuff.append(pred.getDetails());
				// outBuff.append(s);
				// // if (NP.contains("Case 0.1") || NP.contains("Case
				// // 0.0") || NP.contains("Case 0.5"))
				// // NP =
				// // CEModel.adaptationComponent(AssignmentArr[p]);
				// repeat--;
				//
				// }
				// }
